...
// @desc    Get all remarks (admin access)
// @route   GET /api/admin/remarks
// @access  Private/Admin
exports.getAllRemarksForAdmin = async (req, res) => {
  try {
    // Check if user is admin (assuming req.user.isAdmin exists)

    // Fetch all remarks from all users
    const remarks = await Remark.find().sort({ date: -1 }).populate('user', 'firstName lastName email');
